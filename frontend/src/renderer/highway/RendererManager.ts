...
// Supports a named registry so renderers can be switched on the fly.

import type { Renderer, DrawHook, RenderBundle } from './types.js';
import { DefaultRenderer } from './DefaultRenderer.js';

const MAX_DRAW_FAILURES = 3;

type EmitFn = (event: string, detail?: unknown) => void;
type RendererEntry = Renderer | (() => Renderer);
...
export class RendererManager {
  ...
  // ── Registry ──────────────────────────────────────────────────────────
  private registry = new Map<string, RendererEntry>();
  ...
  /** Register a renderer (or lazy factory) under a unique name. */
  register(name: string, entry: RendererEntry): void {
    this.registry.set(name, entry);
    this.emit('renderer:registered', { name });
  }

  /** Remove a registered renderer by name. If it's active, reverts to default. */
  unregister(name: string): void {
    ...
    this.emit('renderer:unregistered', { name });
  }

  /** Switch to a registered renderer by name. Use 'default' for the built-in 2D renderer. */
  switchTo(name: string): void {
    if (name === 'default') {
      this._activeId = 'default';
      this.setRenderer(null);
      this.emit('renderer:switched', { name: 'default' });
      return;
    }
    const entry = this.registry.get(name);
    if (!entry) {
      console.error(`[RendererManager] no renderer registered as '${name}'`);
      return;
    }
    const r = typeof entry === 'function' ? entry() : entry;
    // Replace the lazy factory with the resolved instance for subsequent switches
    if (typeof entry === 'function') this.registry.set(name, r);
    this._activeId = name;
    this.setRenderer(r);
    this.emit('renderer:switched', { name });
  }

  /** Get the name of the currently active renderer. */
  get activeId(): string { return this._activeId; }

  /** List all registered renderer names (plus 'default'). */
  getRegistered(): string[] {
    return ['default', ...this.registry.keys()];
  }

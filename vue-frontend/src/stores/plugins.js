...
import { fetchPlugins, fetchStartupStatus, updatePlugin as apiUpdate } from '@/api/plugins'
...
export const usePluginsStore = defineStore('plugins', () => {
  ...
  async function _waitForStartup(timeoutMs = 30_000) {
    const deadline = Date.now() + timeoutMs
    while (Date.now() < deadline) {
      try {
        const st = await fetchStartupStatus()
